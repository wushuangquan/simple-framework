...
public abstract class AbstractBeanDefinition extends BeanMetadataAttributeAccessor
	...
	public void validate() throws BeanDefinitionValidationException {
		//校验 <bean>标签配置的 lookup属性和replaceMethod属性存在时，对应的重写方法是否存在并且合法
		if (hasMethodOverrides() && getFactoryMethodName() != null) {
			throw new BeanDefinitionValidationException(
					"Cannot combine factory method with container-generated method overrides: " +
					"the factory method must create the concrete bean instance.");
		}
		//如果有 lookup属性或replaceMethod属性
		if (hasBeanClass()) {
			//lookup属性或replaceMethod属性存在时一般会有重载方法，调用时需要运行时确认
			//这里检查对应类中只有一个对应方法即没有重载方法时进行标记，
			// 在使用 CGLIB 增强阶段就不需要做特殊的处理，
			//直接找到某个方法进行增强，否则在增强阶段还需要做特殊处理
			//将重载标记为未重载，以避免arg类型检查的开销。
	...
	public void prepareMethodOverrides() throws BeanDefinitionValidationException {
		...
		//lookup属性或replaceMethod属性存在时一般会有重载方法，调用时需要运行时确认
		//这里检查对应类中只有一个对应方法即没有重载方法时进行标记，避免不必要的多态调用
		//将重载标记为未重载，以避免arg类型检查的开销。
