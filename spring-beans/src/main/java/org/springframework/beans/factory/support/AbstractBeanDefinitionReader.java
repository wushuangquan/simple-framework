...
public abstract class AbstractBeanDefinitionReader implements BeanDefinitionReader, EnvironmentCapable {
	...
	/**
	 * 构造函数中传入 BeanDefinitionRegistry
	...
	protected AbstractBeanDefinitionReader(BeanDefinitionRegistry registry) {
		...
		//BeanDefinitionRegistry 实现类实例有可能是  ResourceLoader
		if (this.registry instanceof ResourceLoader) {
			this.resourceLoader = (ResourceLoader) this.registry;
		} else {
			//PathMatchingResourcePatternResolver 多配置文件的加载
	...
	public int loadBeanDefinitions(String location, @Nullable Set<Resource> actualResources) throws BeanDefinitionStoreException {
		...
		//如果是 ResourcePatternResolver 则是加载多个资源
		if (resourceLoader instanceof ResourcePatternResolver) {
			// Resource pattern matching available.
			try {
				//将Ant 表达式解析出多个资源后，调用 loadBeanDefinitions 加载 多个Resource 的 BeanDefinition
			...
			//调用 loadBeanDefinitions 加载 单个Resource 的 BeanDefinition
