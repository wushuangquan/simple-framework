...
public class XmlBeanDefinitionReader extends AbstractBeanDefinitionReader {
	...
	public int loadBeanDefinitions(EncodedResource encodedResource) throws BeanDefinitionStoreException {
		...
		//支持多线程加载，加载是一个耗时的操作，所以
		//ThreadLocal<Set<EncodedResource>> 存储正在加载的资源
		Set<EncodedResource> currentResources = this.resourcesCurrentlyBeingLoaded.get();
		if (currentResources == null) {
			currentResources = new HashSet<>(4);
			this.resourcesCurrentlyBeingLoaded.set(currentResources);
		}
		//将当前正在加载的资源存储到 currentResources 中
	...
	protected int doLoadBeanDefinitions(InputSource inputSource, Resource resource)
		...
		try {
			//使用 Java 的 SAX 解析 XML成为Document
			// 创建DOM对象，XML的文档对象就是 dom树
			//使用这个Document可以获取文件中的节点并创建节点
			//SAX XML
			Document doc = doLoadDocument(inputSource, resource);
			//解析 DOM树，解析出一个个属性，保存到 BeanDefinition中
	...
	protected BeanDefinitionDocumentReader createBeanDefinitionDocumentReader() {
		//调用工具类 根据传入的 documentReaderClass 实例化 BeanDefinitionDocumentReader
